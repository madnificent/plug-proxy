    ...
    quote do
      ...
      def write(conn, client, opts) do
        case Plug.Conn.read_body(conn, []) do
          {:ok, body, conn} ->
            {body, conn} = PlugProxy.Steps.execute(conn, :body_processor, [body])
            :hackney.send_body(client, body)
            :hackney.finish_send_body(client)
            conn

          {:more, body, conn} ->
            {body, conn} = PlugProxy.Steps.execute(conn, :chunk_processor, [body])
            :hackney.send_body(client, body)
            write(conn, client, opts)
      ...
      def read(conn, client, _) do
        case :hackney.start_response(client) do
          {:ok, status, headers, client} ->
            {headers, conn} = PlugProxy.Steps.execute(conn, :header_processor, [headers, conn])
